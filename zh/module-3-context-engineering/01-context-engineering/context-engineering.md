...
进行 post-handle，通常不是等到 context window 被彻底塞满，而是在系统接近一个「上下文腐烂阈值」时就提前介入。

这里的“上下文腐烂”并不是指窗口容量的物理上限，而是一种更早出现的性能边际递减现象。
...
所以，上下文腐烂阈值更像一个工程上的预警线：

```text
再继续原样累积上下文，模型的下一步决策质量就要开始明显下降了。
```

这也是为什么自主智能体比普通 chat 更容易撞上这个问题。

原因不是它“聊得更久”，而是它更容易遭遇 tool call explosion。

以 Manus、Codex、Deep Research 这一类 agent 为例，频繁的 `web_search`、文件读取、命令执行、页面抓取，都会产生海量 observation。这些 observation 往往还是高密度 token 负载：日志长、细节多、局部有用、整体冗余。
...
```text
上下文腐烂阈值不是靠猜的，也不是直接看模型宣传的最大窗口得出来的，而是必须测出来的。
```

比如基座模型宣称有 1M 甚至更大的上下文窗口，这只说明“理论上能塞进去多少 token”，不代表你的 agent 在那个长度上还能稳定工作。

在很多真实系统里，128K 到 200K 往往已经是很关键的退化临界点。但这个数字不能直接照搬，因为它会受到很多因素影响：

- 任务类型是不是以工具调用为主；
- observation 的平均长度和冗余度；
- prompt 前缀是否稳定；
- 模型本身的长上下文能力；
- 你的 harness 是否已经做了压缩、检索和卸载。
...
只有测出来，你才能给系统设一个真正可用的 pre-rot threshold，而不是靠经验拍一个数字。

你可以把它理解成：不是“上下文满了才压缩”，而是“上下文开始腐烂之前就压缩”。

一旦触发这个阈值，后面的几类动作才开始变得必要：
...
换句话说，post-handle 的时机判断，本身就是 Context Engineering 的一部分。
